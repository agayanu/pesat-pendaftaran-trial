    ...
    <div class="card-body">
        <div class="table-responsive">
            <table class="table">
                ...
                <tbody>
                @foreach($arank as $ark)
                <tr>
                    <td>{{$ark->name}}</td>
                    <td>{{ empty($ark->updated_at) ? $ark->created_at : $ark->updated_at }}</td>
                    <td>
                        <button class="btn btn-sm btn-success" type="button" data-coreui-toggle="modal" data-coreui-target="#edit" data-coreui-id="{{$ark->id}}" data-coreui-nama="{{$ark->name}}"><i class="cil-pen" style="font-weight:bold"></i></button>
                        <button class="btn btn-sm btn-danger" type="button" data-coreui-toggle="modal" data-coreui-target="#hapus" data-coreui-nama="{{$ark->name}}" data-coreui-url="{{url('prestasi-rank/'.$ark->id)}}"><i class="cil-trash" style="font-weight:bold"></i></button>
                    </td>
                </tr>
                @endforeach
                </tbody>
...
<div class="modal fade" id="edit" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ route('prestasi-rank-perbarui') }}" method="post" class="validation-update" novalidate>
                @csrf
