    ...
    <div class="card-body">
        <div class="table-responsive">
            <table class="table">
                ...
                <tr>
                    <td>{{$bld->name}}</td>
                    <td>{{ empty($bld->updated_at) ? $bld->created_at : $bld->updated_at }}</td>
                    <td>
                        <button class="btn btn-sm btn-success" type="button" data-coreui-toggle="modal" data-coreui-target="#edit" data-coreui-id="{{$bld->id}}" data-coreui-nama="{{$bld->name}}"><i class="cil-pen" style="font-weight:bold"></i></button>
                        <button class="btn btn-sm btn-danger" type="button" data-coreui-toggle="modal" data-coreui-target="#hapus" data-coreui-nama="{{$bld->name}}" data-coreui-url="{{url('golongan-darah/'.$bld->id)}}"><i class="cil-trash" style="font-weight:bold"></i></button>
                    </td>
                </tr>
                @endforeach
                </tbody>
...
<div class="modal fade" id="edit" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ route('golongan-darah-perbarui') }}" method="post" class="validation-update" novalidate>
                @csrf
