    ...
    <div class="card-body">
        <div class="table-responsive">
            <table class="table">
                ...
                <tbody>
                @foreach($alevel as $alv)
                <tr>
                    <td>{{$alv->name}}</td>
                    <td>{{ empty($alv->updated_at) ? $alv->created_at : $alv->updated_at }}</td>
                    <td>
                        <button class="btn btn-sm btn-success" type="button" data-coreui-toggle="modal" data-coreui-target="#edit" data-coreui-id="{{$alv->id}}" data-coreui-nama="{{$alv->name}}"><i class="cil-pen" style="font-weight:bold"></i></button>
                        <button class="btn btn-sm btn-danger" type="button" data-coreui-toggle="modal" data-coreui-target="#hapus" data-coreui-nama="{{$alv->name}}" data-coreui-url="{{url('prestasi-level/'.$alv->id)}}"><i class="cil-trash" style="font-weight:bold"></i></button>
                    </td>
                </tr>
                @endforeach
                </tbody>
...
<div class="modal fade" id="edit" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ route('prestasi-level-perbarui') }}" method="post" class="validation-update" novalidate>
                @csrf
