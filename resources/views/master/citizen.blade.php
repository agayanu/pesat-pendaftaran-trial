    ...
    <div class="card-body">
        <div class="table-responsive">
            <table class="table">
                ...
                <tr>
                    <td>{{$ctz->name}}</td>
                    <td>{{ empty($ctz->updated_at) ? $ctz->created_at : $ctz->updated_at }}</td>
                    <td>
                        <button class="btn btn-sm btn-success" type="button" data-coreui-toggle="modal" data-coreui-target="#edit" data-coreui-id="{{$ctz->id}}" data-coreui-nama="{{$ctz->name}}"><i class="cil-pen" style="font-weight:bold"></i></button>
                        <button class="btn btn-sm btn-danger" type="button" data-coreui-toggle="modal" data-coreui-target="#hapus" data-coreui-nama="{{$ctz->name}}" data-coreui-url="{{url('kewarganegaraan/'.$ctz->id)}}"><i class="cil-trash" style="font-weight:bold"></i></button>
                    </td>
                </tr>
                @endforeach
                </tbody>
...
<div class="modal fade" id="edit" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ route('kewarganegaraan-perbarui') }}" method="post" class="validation-update" novalidate>
                @csrf
