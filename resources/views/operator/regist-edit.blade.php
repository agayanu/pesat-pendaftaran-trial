...
<div class="card mb-3">
    <div class="card-header">
        <div class="d-flex justify-content-between">
            <div>No. Daftar : <b>{{$dd->no_regist}}</b></div>
            <a class="btn btn-sm btn-secondary" href="{{route('pendaftaran')}}">Kembali</a>
        </div>
    </div>
    <div class="card-body">
        <form action="{{route('pendaftaran-perbarui')}}" method="post" class="validation-create" novalidate>
        @csrf
            <div class="row">
                <div class="col-sm-3 mb-3">
                    <label class="form-label">Tanggal <div class="required">*</div></label>
                    <input type="hidden" name="id" value="{{$dd->id}}">
                    <input type="hidden" name="no_regist" value="{{$dd->no_regist}}">
                    <input type="text" class="form-control @error('created_at') is-invalid @enderror" name="created_at" id="created_at" placeholder="dd-mm-yyyy" value="{{old('created_at') ?? date_format(date_create($dd->created_at),'d-m-Y')}}" required>
                    @error('created_at')
                ...
                <div class="col-sm-3 mb-3">
                    ...
                    <select name="phase" class="form-select @error('phase') is-invalid @enderror" required>
                        @foreach($phase as $p)
                            <option value="{{$p->id}}" {{old('phase') ?? $dd->phase == $p->id ? 'selected' : ''}}>{{$p->name}}</option>
                        @endforeach
                    </select>
                    @error('phase')
                    <div class="invalid-feedback">{{ $message }}</div>
                ...
                <div class="col-sm-3 mb-3">
                    ...
                    <select name="grade" class="form-select @error('grade') is-invalid @enderror" required>
                        ...
                        @foreach($grade as $g)
                            <option value="{{$g->id}}" {{old('grade') ?? $dd->grade == $g->id ? 'selected' : ''}}>{{$g->name}}</option>
                        @endforeach
                    </select>
                    @error('grade')
                    <div class="invalid-feedback">{{ $message }}</div>
                ...
                <div class="col-sm-3 mb-3">
                    ...
                    <select name="major" class="form-select @error('major') is-invalid @enderror" required>
                        <option value="">--pilih--</option>
                        @foreach($major as $m)
                            <option value="{{$m->id}}" {{old('major') ?? $dd->major == $m->id ? 'selected' : ''}}>{{$m->name}}</option>
                        @endforeach
                    </select>
                    @error('major')
                    <div class="invalid-feedback">{{ $message }}</div>
            ...
            <div class="row">
                <div class="col-sm-6 mb-3">
                    ...
                    <input type="text" class="form-control @error('name') is-invalid @enderror" name="name" value="{{old('name') ?? $dd->name}}" required>
                    @error('name')
                    <div class="invalid-feedback">{{ $message }}</div>
                    @else
                ...
                <div class="col-sm-6 mb-3">
                    ...
                    <label class="switch">
                        <input type="checkbox" name="school_check" id="school_check" {{!empty(old('school') ?? $dd->school) ? 'checked' : ''}}>
                        <span class="slider round"></span>
                    </label>
                    <select name="school" id="school" class="form-select @error('school') is-invalid @enderror">
                        <option value="">Cari Sekolah</option>
                        @if (old('school') ?? $dd->school)
                            @php
                                $schoolOldId = old('school') ?? $dd->school;
                                $schoolOld   = DB::table('tm_schools')->select('id','name')->where('id',$schoolOldId)->first();
                            @endphp
                            <option value="{{$schoolOld->id}}" selected>{{$schoolOld->name}}</option>
                        @endif
                    </select>
                    @error('school')
                    <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                    <input type="text" class="form-control @error('remark') is-invalid @enderror" name="remark" id="remark" value="{{old('remark') ?? $dd->remark}}" placeholder="Nama Sekolah">
            ...
            <div class="row">
                <div class="col-sm-6 mb-3">
                    ...
                    <input type="email" class="form-control @error('email') is-invalid @enderror" name="email" value="{{old('email') ?? $dd->email}}" required>
                    @error('email')
                    <div class="invalid-feedback">{{ $message }}</div>
                    @else
                ...
                <div class="col-sm-3 mb-3">
                    ...
                    <select name="gender" class="form-select @error('gender') is-invalid @enderror" required>
                        ...
                        @foreach($gender as $gn)
                            <option value="{{$gn->id}}" {{old('gender') ?? $dd->gender == $gn->id ? 'selected' : ''}}>{{$gn->name}}</option>
                        @endforeach
                    </select>
                    @error('gender')
                    <div class="invalid-feedback">{{ $message }}</div>
                ...
                <div class="col-sm-3 mb-3">
                    ...
                    <select name="religion" class="form-select @error('religion') is-invalid @enderror" required>
                        ...
                        @foreach($religion as $r)
                            <option value="{{$r->id}}" {{old('religion') ?? $dd->religion == $r->id ? 'selected' : ''}}>{{$r->name}}</option>
                        @endforeach
                    </select>
                    @error('religion')
                    <div class="invalid-feedback">{{ $message }}</div>
            ...
            <div class="row">
                <div class="col-sm-6 mb-3">
                    ...
                    <input type="text" class="form-control @error('parent') is-invalid @enderror" name="parent" value="{{old('parent') ?? $dd->parent}}" required>
                    @error('parent')
                    <div class="invalid-feedback">{{ $message }}</div>
                    @else
                ...
                <div class="col-sm-3 mb-3">
                    ...
                    <input type="text" class="form-control @error('hp_parent') is-invalid @enderror" name="hp_parent" maxlength="15" value="{{old('hp_parent') ?? $dd->hp_parent}}" required>
                    @error('parent')
                    <div class="invalid-feedback">{{ $message }}</div>
                    @else
                ...
                <div class="col-sm-3 mb-3">
                    ...
                    <input type="text" class="form-control @error('hp') is-invalid @enderror" name="hp" maxlength="15" value="{{old('hp') ?? $dd->hp}}" required>
                    @error('hp')
                    <div class="invalid-feedback">{{ $message }}</div>
                    @else
...
$(document).ready(function () {
    ...
    var schoolOld = {{old('school') ?? $dd->school ?? 'null'}};
    if(schoolOld === null)
    {
        $('#school').next('.select2-container').hide();
    }
