            ...
            <div class="card-body">
                ...
                @csrf
                    <div class="row">
                        ...
                        <div class="col-sm-12 mb-3">
                            <label class="form-label">Perubahan <div class="required">*</div></label>
                            <select name="change_cost" class="form-select @error('change_cost') is-invalid @enderror" required>
                                <option value="">--pilih--</option>
                                @foreach ($changeCost as $cc)
                                    <option value="{{$cc->id}}">{{$cc->phase}} {{$cc->grade_from}} {{$cc->major_from}} ({{number_format($cc->amount_from)}}) Ke {{$cc->grade_to}} {{$cc->major_to}} ({{number_format($cc->amount_to)}})</option>
                                @endforeach
                            </select>
                            @error('change_cost')
                            <div class="invalid-feedback">{{ $message }}</div>
                            @else
                            <div class="invalid-feedback">Perubahan Wajib Diisi!</div>
                            @enderror
                        </div>
                    </div>
...
$(document).ready(function () {
    $("#id_regist").select2({
        ajax: {
            url: "{{route('rubah-bayar-search', ['period_select'=>$ps])}}".replace(/&amp;/g, "&"),
            type: 'POST',
            dataType: 'json',
            delay: 250,
